...
export default function ChatRoomScreen({ route, navigation }) {
  ...
  const [pinnedMessage, setPinnedMessage] = useState(
    'Welcome to Global Nax Room! Be respectful.'
  );
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Message send nahi ho paya.');
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(
        doc(db, 'global_chats', selectedMessage.id)
      );
    } catch (error) {
      console.log('Delete error:', error);
      Alert.alert('Error', 'Message delete nahi ho paya.');
    }
  ...
  const handleAction = async (action) => {
    ...
    if (action === 'copy') {
      try {
        ...
        Alert.alert('Error', 'Text copy nahi ho paya.');
      }
      ...
      Alert.alert('Star', 'Message starred.');
    } else if (action === 'translate') {
      Alert.alert('Translate', 'Translation feature opening...');
    } else if (action === 'edit') {
      Alert.alert('Edit', 'Edit feature opening...');
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <Image
            source={{
              uri: 'https://ui-avatars.com/api/?name=Nax+Room&background=007AFF&color=fff'
            }}
            style={styles.avatar}
          />
