...
export default function ChatRoomScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem('nax_unique_id').then(id => { if (id && mountedRef.current) setMyUniqueId(id); });
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, 'users', friendId), snap => {
      if (mountedRef.current) setOnline(Boolean(snap.data()?.online));
    });
  }, [friendId]);
  ...
  const animateSend = () => {
    ...
    if (typeof Haptics !== 'undefined' && Haptics.impactAsync) Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
  };
    ...
    try {
      if (editMsgId) {
        ...
        if (chatId !== 'global_chats' && friendId) {
          BotService.getBotById(friendId).then(bot => {
            if (bot) {
              setTimeout(() => {
                BotMessageHandler.processMessage(chatId, friendId, val, currentUser.displayName || 'User');
              }, 600);
            }
          }).catch(e => console.log('Bot check error:', e));
        }
      }
    } catch (e) { Alert.alert('Error', 'Message send failed.'); }
  };

  // ---> NEW: HANDLE BOT INLINE BUTTON CLICK <---
  const handleBotButtonPress = async (actionText) => {
    if (!currentUser?.uid) return;
    const colPath = chatId === 'global_chats' ? 'global_chats' : `chats/${chatId}/messages`;
    
    try {
      // 1. Instantly send the user's action as a message
      ...
      if (chatId !== 'global_chats' && friendId) {
        BotService.getBotById(friendId).then(bot => {
          if (bot) {
            setTimeout(() => {
              BotMessageHandler.processMessage(chatId, friendId, actionText, currentUser.displayName || 'User');
            }, 400); // Fast reply for button clicks
          }
        }).catch(e => console.log('Bot button trigger error:', e));
      }
    } catch (error) {
      console.log('Bot button press error:', error);
    }
  };
  ...
  const uploadToCloud = async (file, msgType) => {
    ...
    try {
      ...
      if (json.status === 'success') {
        ...
        if (typeof Haptics !== 'undefined') Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  ...
  const acceptCall = async () => {
    ...
    const { id, type } = incomingCall; setCallType(type); setCallVisible(true); setCallState('connecting'); setIncomingCall(null); setCallError('');
    const stream = await getMedia(type); if (!stream) return cleanupCall(false);
    localStreamRef.current = stream; setLocalStream(stream);
    const peer = await createPeer(); stream.getTracks().forEach(t => peer.addTrack(t, stream));
  ...
  const openAttachmentMenu = () => { Keyboard.dismiss(); setShowAttachments(true); Animated.spring(attachAnim, { toValue: 0, friction: 7, useNativeDriver: true }).start(); if(typeof Haptics !== 'undefined') Haptics.selectionAsync(); };
  ...
  const handleReaction = async (emoji) => {
    const msg = contextMenu.msg; setContextMenu({ visible: false, msg: null, yPos: 0 }); if(typeof Haptics !== 'undefined') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    ...
    if (item.isDeleted) {
      return (
        <View style={[styles.msgRow, { justifyContent: isMe ? 'flex-end' : 'flex-start' }]}>
          <View style={[styles.bubble, { backgroundColor: 'transparent', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }]}><Text style={{ color: '#8FA6B9', fontStyle: 'italic' }}>🚫 Message deleted</Text></View>
        </View>
      );
    }

    return (
      <Pressable onLongPress={(e) => { Keyboard.dismiss(); setContextMenu({ visible: true, msg: item, yPos: e.nativeEvent.pageY }); if(typeof Haptics !== 'undefined') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy); }}>
        <View style={[styles.msgRow, { justifyContent: isMe ? 'flex-end' : 'flex-start' }]}>
          <View style={[styles.bubble, isMe ? styles.myBubble : styles.otherBubble]}>
            
            {!isMe && (chatId === 'global_chats' || item.isBot) && item.senderName && (
              <Text style={styles.senderName}>{item.senderName}</Text>
            )}

            {(item.replyToId || item.replyToText) && (
              <View style={styles.replyBox}>
                <Text style={{ color: COLORS.bubbleMe, fontSize: 11, fontWeight: 'bold' }}>{item.replyToSender || 'User'}</Text>
                <Text style={{ color: COLORS.sub, fontSize: 12 }} numberOfLines={1}>{item.replyToText || 'Media'}</Text>
              </View>
            )}
            ...
            {/* ---> RENDERING BOT BUTTONS (TELEGRAM STYLE) <--- */}
            {item.botButtons && item.botButtons.length > 0 && (
              <View style={styles.botButtonsContainer}>
                {item.botButtons.map((btn, index) => (
                  <TouchableOpacity 
                    key={index}
                    style={styles.botInlineButton}
                    onPress={() => handleBotButtonPress(btn.action)}
                  >
                    <Text style={styles.botInlineButtonText}>{btn.text}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            )}
        ...
        <Modal visible animationType="slide">
          <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }}>
            ...
            <View style={styles.callControls}>
              <TouchableOpacity style={styles.ctrlBtn} onPress={toggleMic}><Ionicons name={micEnabled ? "mic" : "mic-off"} size={26} color="#FFF"/></TouchableOpacity>
              {callType === 'video' && <TouchableOpacity style={styles.ctrlBtn} onPress={toggleCamera}><Ionicons name={cameraEnabled ? "videocam" : "videocam-off"} size={26} color="#FFF"/></TouchableOpacity>}
...
const styles = StyleSheet.create({
  ...
  senderName: { color: '#AF52DE', fontSize: 12, fontWeight: 'bold', marginBottom: 4 },
  ...
  // ---> BOT BUTTON STYLES <---
  botButtonsContainer: { marginTop: 10, flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  botInlineButton: { backgroundColor: 'rgba(79, 195, 255, 0.15)', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(79, 195, 255, 0.3)' },
  botInlineButtonText: { color: '#4FC3FF', fontSize: 13, fontWeight: '700' },
