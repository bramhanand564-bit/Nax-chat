...
export default function ChatRoomScreen({ route, navigation }) {
  ...
  const [pinnedMessage, setPinnedMessage] = useState("Welcome to Global Nax Room! Start chatting.");
  ...
  // 🔴 NEW: Poll सेंड करने का फंक्शन
  const sendPoll = async () => {
    if (!pollQuestion.trim() || pollOptions.some(opt => !opt.trim())) {
      Alert.alert("Incomplete", "Please enter a question and all options.");
      return;
    }
    setShowPollModal(false);
    setShowAttachments(false);

    const formattedOptions = pollOptions.map((opt, index) => ({
      id: index.toString(), text: opt, voters: [] // खाली वोटर्स लिस्ट
    }));
  ...
  const handleAction = (action) => {
    ...
    else if (action === 'pin') setPinnedMessage(selectedMessage.text || (selectedMessage.type === 'poll' ? '📊 Poll' : '📷 Photo'));
    setSelectedMessage(null);
  };
    ...
    return (
      <View key={msg.id} style={[styles.messageRow, { justifyContent: isMe ? 'flex-end' : 'flex-start' }]}>
        <TouchableOpacity 
          ...
          style={[styles.messageBubble, { backgroundColor: isMe ? myMsgBg : otherMsgBg, padding: msg.type === 'image' ? 4 : 8 }]}
        >
          ...
          {/* 🔴 NEW: Live Poll UI */}
          {msg.type === 'poll' ? (
            <View style={{ minWidth: 200, paddingVertical: 5 }}>
              ...
              <Text style={{ color: textSub, fontSize: 11, marginTop: 5, textAlign: 'right' }}>
                {msg.options.reduce((acc, o) => acc + o.voters.length, 0)} votes
              </Text>
            </View>
      ...
      <View style={[styles.container, { backgroundColor: bg }]}>
        ...
        <View style={[styles.header, { backgroundColor: headerBg }]}>
          ...
          <View style={styles.headerInfo}>
            ...
            <Text style={styles.headerStatus}>10.5K members</Text>
          </View>
        ...
        <ScrollView style={styles.chatArea} contentContainerStyle={{ padding: 10, paddingBottom: 20 }} ref={scrollViewRef} onContentSizeChange={() => scrollViewRef.current?.scrollToEnd({ animated: true })}>
          <View style={styles.encryptionBox}><Ionicons name="lock-closed" size={12} color="#F5C518" /><Text style={styles.encryptionText}>Messages and calls are end-to-end encrypted.</Text></View>
          ...
          <View style={[styles.attachmentTray, { backgroundColor: headerBg }]}>
            <View style={styles.attachGrid}>
              <TouchableOpacity style={styles.attachOption} onPress={takePhoto}>
                <View style={[styles.attachIconWrap, { backgroundColor: '#D3396D' }]}><Ionicons name="camera" size={24} color="#FFF" /></View>
                <Text style={[styles.attachText, { color: textMain }]}>Camera</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.attachOption} onPress={pickImage}>
                <View style={[styles.attachIconWrap, { backgroundColor: '#007AFF' }]}><Ionicons name="image" size={24} color="#FFF" /></View>
                <Text style={[styles.attachText, { color: textMain }]}>Gallery</Text>
              </TouchableOpacity>
              {/* 🔴 NEW: POLL BUTTON */}
              <TouchableOpacity style={styles.attachOption} onPress={() => { setShowAttachments(false); setShowPollModal(true); }}>
                <View style={[styles.attachIconWrap, { backgroundColor: '#F59E0B' }]}><Ionicons name="bar-chart" size={24} color="#FFF" /></View>
                <Text style={[styles.attachText, { color: textMain }]}>Poll</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.attachOption} onPress={() => Alert.alert("Location", "Maps API connecting...")}>
                <View style={[styles.attachIconWrap, { backgroundColor: '#25D366' }]}><Ionicons name="location" size={24} color="#FFF" /></View>
                <Text style={[styles.attachText, { color: textMain }]}>Location</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
        ...
        {replyingTo && (
          <View style={[styles.replyingBar, { backgroundColor: headerBg }]}>
            <View style={{ flex: 1 }}><Text style={{ color: '#007AFF', fontWeight: 'bold', fontSize: 13 }}>Replying to {replyingTo.senderName}</Text><Text style={{ color: textSub, fontSize: 13 }} numberOfLines={1}>{replyingTo.text || '📷 Photo'}</Text></View>
            <TouchableOpacity onPress={() => setReplyingTo(null)}><Ionicons name="close-circle" size={24} color={textSub} /></TouchableOpacity>
          </View>
        )}
        ...
        <View style={[styles.inputArea, { backgroundColor: bg }]}>
          ...
          <TouchableOpacity style={[styles.micBtn, { backgroundColor: inputText.trim() ? '#007AFF' : '#00A884' }]} onPress={inputText.trim() ? sendMessage : () => Alert.alert("Voice Record", "Hold to record audio")}>
            <Ionicons name={inputText.trim() ? "send" : "mic"} size={20} color="#FFF" style={inputText.trim() ? {marginLeft: 4} : {}} />
          </TouchableOpacity>
        </View>

        {/* 🔴 NEW: POLL CREATOR MODAL 🔴 */}
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.pollModalContent, { backgroundColor: inputBg }]}>
              ...
              {pollOptions.map((opt, index) => (
                <TextInput key={index} style={[styles.pollInput, { backgroundColor: headerBg, color: textMain, marginTop: 10 }]} placeholder={`Option ${index + 1}`} placeholderTextColor={textSub} value={opt} onChangeText={(text) => { const newOpts = [...pollOptions]; newOpts[index] = text; setPollOptions(newOpts); }} />
              ))}

              {pollOptions.length < 4 && (
                <TouchableOpacity style={{ marginTop: 15, paddingVertical: 10 }} onPress={() => setPollOptions([...pollOptions, ''])}>
                  <Text style={{ color: '#007AFF', fontWeight: 'bold' }}>+ Add Option</Text>
                </TouchableOpacity>
              )}
          ...
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setSelectedMessage(null)}>
            <View style={[styles.modalContent, { backgroundColor: inputBg }]}>
              <View style={styles.reactionRow}>
                {['❤️', '😂', '🔥', '👍', '😢'].map((emoji, i) => (
                  <TouchableOpacity key={i} onPress={() => handleReaction(emoji)}><Text style={{ fontSize: 30, marginHorizontal: 8 }}>{emoji}</Text></TouchableOpacity>
                ))}
              </View>
