...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ScrollView, Image, KeyboardAvoidingView, Platform, Alert, Modal, Clipboard, Animated } from 'react-native';
...
export default function ChatRoomScreen({ route, navigation }) {
  ...
  // Theme Colors
  const bg = isDark ? '#0B141A' : '#EFEAE2'; // WhatsApp Chat Background Vibe
  ...
  const handleAction = (action) => {
    if (action === 'copy') {
      ...
      Alert.alert(action, `Feature '${action}' engine will connect here!`);
    }
    setSelectedMessage(null);
  };
      ...
      <View style={[styles.container, { backgroundColor: bg }]}>
        ...
        <View style={[styles.header, { backgroundColor: headerBg }]}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity onPress={() => Alert.alert("Video Call", "Connecting WebRTC Video Engine...")} style={styles.actionIcon}>
              <Ionicons name="videocam" size={22} color={textMain} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => Alert.alert("Voice Call", "Connecting Audio Engine...")} style={styles.actionIcon}>
        ...
        >
          <View style={styles.encryptionBox}>
            ...
            <Text style={styles.encryptionText}>Messages and calls are end-to-end encrypted. No one outside of this chat can read or listen to them.</Text>
          </View>

          {messages.map(renderMessage)}
        </ScrollView>

        {/* ================= MEGA ATTACHMENTS TRAY ================= */}
        {showAttachments && (
            ...
            <View style={[styles.modalContent, { backgroundColor: inputBg }]}>
              ...
              <View style={styles.actionGrid}>
                {[
                  ...
                  if (act.id === 'delete' && selectedMessage?.senderId !== currentUser?.uid) return null; // Hide delete if not my msg
                  return (
